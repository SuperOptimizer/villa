...
[[gnu::always_inline]] static inline uint64_t mix64(uint64_t x) noexcept {
    // SplitMix64
...
[[gnu::always_inline]] static inline double det_jitter01(int y, int x, uint64_t salt) noexcept {
    uint64_t h = mix64((uint64_t(y) << 32) ^ uint64_t(x) ^ salt);
    // map to [0,1)
    constexpr double inv = 1.0 / double(UINT64_C(1) << 53);
    return double(h >> 11) * inv;
}
[[gnu::always_inline]] static inline double det_jitter_symm(int y, int x, uint64_t salt) noexcept {
    // map to ~[-1,1)
    return 2.0 * det_jitter01(y, x, salt) - 1.0;
}
// -----------------------------------------------------------------------------

// FMA-friendly bilinear interpolation: a + t*(b-a) generates fmadd instructions
[[gnu::always_inline]] static cv::Vec3f at_int_inv(const cv::Mat_<cv::Vec3f> &points, cv::Vec2f p) noexcept
{
    const int x = static_cast<int>(p[1]);
    const int y = static_cast<int>(p[0]);
    const float fx = p[1] - x;
    const float fy = p[0] - y;
    ...
    const cv::Vec3f p0 = p00 + fx * (p01 - p00);
    const cv::Vec3f p1 = p10 + fx * (p11 - p10);
...
[[gnu::always_inline]] static inline double maybe_quad_area_and_mark(int j, int i,
                                              const cv::Mat_<uint8_t>& state,
                                              const cv::Mat_<cv::Vec3d>& points,
                                              cv::Mat_<uint8_t>& quad_done) noexcept
{
    if (j < 0 || i < 0 || j >= quad_done.rows || i >= quad_done.cols) [[unlikely]] return 0.0;
    if (quad_done(j,i)) [[unlikely]] return 0.0;
...
public:
    ...
    // [APPROVED] helpers
    bool isApproved(QuadSurface* sm, const cv::Vec2i& p) const {
        return _approved.contains({sm, p});
    }
    void setApproved(QuadSurface* sm, const cv::Vec2i& p) {
        _approved.insert({sm, p});
    }
    void clearApprovedPair(QuadSurface* sm, const cv::Vec2i& p) {
    ...
    bool approvedAt(const cv::Vec2i& p) const {
        ...
        for (const auto& s : S) if (isApproved(s, p)) return true;
        return false;
    }
...
// protected:
    ...
    std::unordered_map<cv::Vec2i, std::set<QuadSurface*>> _surfs;
    ...
    SurfTrackerData &data, ceres::Problem &problem, const cv::Mat_<uint8_t> &state, int flags = 0, float w = 0.7f)
{
    ...
    // std::cout << "add straight " << sm << p << o1 << o2 << o3 << "\n";
    ...
    bool parallel = true)
{
    std::cout << "hr_gen: start used_area=" << used_area << " step=" << step
              << " inpaint=" << inpaint << " parallel=" << (parallel?1:0) << "\n";
                    ...
                    && data.valid_int(sm,{j+1,i+1}))
                {
                    ...
                    for(int sy=0;sy<=step;sy++)
                        for(int sx=0;sx<=step;sx++) {
                            const float fx = sx/step;
                            const float fy = sy/step;
                            // FMA-friendly lerp: a + t*(b-a)
                            const cv::Vec2f l0 = l00 + fx * (l01 - l00);
                            const cv::Vec2f l1 = l10 + fx * (l11 - l10);
                            const cv::Vec2f l = l0 + fy * (l1 - l0);
                ...
                for(int sy=0;sy<=step;sy++)
                    for(int sx=0;sx<=step;sx++) {
                        if (!weights_hr(j*step+sy,i*step+sx)) {
                            ...
                            const float fy = sy/step;
                            // FMA-friendly lerp: a + t*(b-a)
                            const cv::Vec3d c0 = c00 + fx * (c01 - c00);
                            const cv::Vec3d c1 = c10 + fx * (c11 - c10);
                            const cv::Vec3d c = c0 + fy * (c1 - c0);
    ...
    bool remap_parallel = false)
{
    ...
    std::cout << "remap: start used_area=" << used_area << " parallel=" << (remap_parallel?1:0) << "\n";
        ...
        for(int i=used_area.x;i<used_area.br().x;i++)
            if (static_bounds.contains(cv::Point(i,j))) {
                ...
                if (loc_valid(points_hr, l)) {
                    ...
                    std::set<QuadSurface*> approved_neighborhood;
                    ...
                    if (data_out.surfs({j,i}).empty() && !approved_neighborhood.empty()) {
                        QuadSurface* best_s = nullptr;
                        float best_res = std::numeric_limits<float>::max();
                        cv::Vec3f best_loc_raw;
                        for (const auto& s : approved_neighborhood) {
                            auto ptr = s->pointer();
        ...
        for(int j=used_area.y;j<used_area.br().y-1;j++)
            for(int i=used_area.x;i<used_area.br().x-1;i++)
                if (!static_bounds.contains(cv::Point(i,j)) && state_out(j,i) & STATE_LOC_VALID && (fringe(j, i) || fringe_next(j, i))) {
                    ...
                    for(const auto& s : data_out.surfs({j,i})) {
                    ...
                    for(const auto& test_surf : surf_cands) {
                        mutex.lock_shared();
...
QuadSurface *grow_surf_from_surfs(QuadSurface *seed, const std::vector<QuadSurface*> &surfs_v, const TracerParams &params, float voxelsize)
{
    // Timing accumulator for pointTo calls (thread-safe via omp atomic)
    double pointTo_total_ms = 0.0;

    bool flip_x = params.flip_x;
    int global_steps_per_window = params.global_steps_per_window;
    ...
    float src_step = params.src_step;
    float step = params.step;
    const int step_int = surftrack_round_step(step);
    int max_width = params.max_width;

    local_cost_inl_th = params.local_cost_inl_th;
    assoc_surface_th = params.same_surface_th;
    duplicate_surface_th = params.duplicate_surface_th >= 0 ? params.duplicate_surface_th : assoc_surface_th;
    remap_attach_surface_th = params.remap_attach_surface_th >= 0 ? params.remap_attach_surface_th : assoc_surface_th;
    point_to_max_iters = params.point_to_max_iters;
    int point_to_seed_max_iters = params.point_to_seed_max_iters;
    bool use_surface_patch_index = params.use_surface_patch_index;
    int surface_patch_stride = params.surface_patch_stride;
    float surface_patch_bbox_pad = params.surface_patch_bbox_pad;
    straight_weight = params.straight_weight;
    straight_weight_3D = params.straight_weight_3D;
    sliding_w_scale = params.sliding_w_scale;
    z_loc_loss_w = params.z_loc_loss_w;
    dist_loss_2d_w = params.dist_loss_2d_w;
    dist_loss_3d_w = params.dist_loss_3d_w;
    straight_min_count = params.straight_min_count;
    inlier_base_threshold = params.inlier_base_threshold;
    uint64_t deterministic_seed = params.deterministic_seed;
    double deterministic_jitter_px = params.deterministic_jitter_px;

    // [APPROVED] knobs
    bool pin_approved_points = params.pin_approved_points;
    bool keep_approved_on_consistency = params.keep_approved_on_consistency;
    bool prefer_approved_in_hr = params.prefer_approved_in_hr;
    float approved_weight_hr = params.approved_weight_hr;

    // Optional hard z-range constraint: [z_min, z_max]
    bool enforce_z_range = false;
    double z_min = 0.0, z_max = 0.0;
    if (params.z_range) {
        z_min = (*params.z_range)[0];
        z_max = (*params.z_range)[1];
        if (z_min > z_max)
            std::swap(z_min, z_max);
        enforce_z_range = true;
    } else if (params.z_min >= 0 && params.z_max < std::numeric_limits<int>::max()) {
        z_min = params.z_min;
        z_max = params.z_max;
        if (z_min > z_max)
            std::swap(z_min, z_max);
    ...
    std::cout << "  assoc_surface_th: " << assoc_surface_th << "\n";
    std::cout << "  duplicate_surface_th: " << duplicate_surface_th << "\n";
    std::cout << "  remap_attach_surface_th: " << remap_attach_surface_th << "\n";
    std::cout << "  point_to_max_iters: " << point_to_max_iters << "\n";
    std::cout << "  point_to_seed_max_iters: " << point_to_seed_max_iters << "\n";
    std::cout << "  use_surface_patch_index: " << use_surface_patch_index << "\n";
    std::cout << "  surface_patch_stride: " << surface_patch_stride << "\n";
    std::cout << "  surface_patch_bbox_pad: " << surface_patch_bbox_pad << "\n";
    ...
    std::cout << "  deterministic_seed: " << deterministic_seed << "\n";
    std::cout << "  deterministic_jitter_px: " << deterministic_jitter_px << "\n";
    if (enforce_z_range)
        std::cout << "  z_range: [" << z_min << ", " << z_max << "]" << "\n";
    std::cout << "  pin_approved_points: " << pin_approved_points << "\n";
    std::cout << "  keep_approved_on_consistency: " << keep_approved_on_consistency << "\n";
    std::cout << "  prefer_approved_in_hr: " << prefer_approved_in_hr << "\n";
    std::cout << "  approved_weight_hr: " << approved_weight_hr << "\n";
    ...
    for(auto &sm : surfs_v) {
        if (sm->meta.tags.approved.has_value())
            approved_sm.insert(sm);
        if (!sm->meta.tags.defective.has_value()) {
            surfs[sm->id] = sm;
        }
    }

    for(const auto& sm : approved_sm)
    ...
    std::cout << "seed " << seed << " name " << seed->id << " seed overlapping: "
              << overlapping_map[seed].size() << "/" << seed->overlappingIds().size() << "\n";
    ...
    // Deterministic seed search around center using PRNG seeded by param
    {
        ...
        if (tries > 0) {
            std::cout << "deterministic seed search tries: " << tries << " got " << seed_loc << "\n";
        }
    }
    ...
    if (enforce_z_range && (data.seed_coord[2] < z_min || data.seed_coord[2] > z_max))
        std::cout << "warning: seed z " << data.seed_coord[2] << " is outside z_range; growth will be restricted to [" << z_min << ", " << z_max << "]" << "\n";
    ...
    for(const auto& p : fringe) {
        ...
        std::cout << "testing " << p << " from cands: " << overlapping_map[seed].size() << coord << "\n";
        for(const auto& s : overlapping_map[seed]) {
            auto ptr = s->pointer();
            auto _t0 = std::chrono::steady_clock::now();
    ...
    for(int generation=0;generation<stop_gen;generation++) {
        ...
        const int approved_priority_radius = params.approved_priority_radius;
...
#pragma omp parallel for schedule(static)
        for (int idx = 0; idx < static_cast<int>(vec.size()); ++idx) {
            ...
            if (misses) {
                std::cout << "grow: cleaned " << misses << " stale surface refs in thread-local cache" << "\n";
            }
            mutex.unlock_shared();
            mutex.lock();
            ...
            std::set<QuadSurface*> test_surfs = local_surfs;
            if (params.consider_all_approved_as_candidates) {
            ...
            for(const auto& ref_surf : local_surfs) {
                int ref_count = 0;
                cv::Vec2d avg = {0,0};
                bool ref_seed = false;
                for(int oy=std::max(p[0]-r,0);oy<=std::min(p[0]+r,h-1);oy++)
                ...
                int min_straight = params.approved_min_straight_in_grow;
                int min_count    = params.approved_min_count_in_grow;
                ...
                for(const auto& test_surf : test_surfs) {
                    auto ptr = test_surf->pointer();
            ...
            if (best_inliers >= curr_best_inl_th || best_ref_seed) {
                ...
                for(const auto& test_surf : test_surfs) {
                    for(const auto& s : overlapping_map[test_surf])
...
#pragma omp critical
        ...
        int inl_lower_bound_reg = params.consensus_default_th;
        int inl_lower_bound_b = params.consensus_limit_th;
        ...
        if (generation % 50 == 0 || update_mapping /*|| generation < 10*/) {
            {
                cv::Mat_<cv::Vec3f> points_hr =
                    surftrack_genpoints_hr(data, state, points, used_area, step, src_step,
                                           /*inpaint=*/false, approved_weight_hr, prefer_approved_in_hr,
                                           /*parallel=*/params.hr_gen_parallel);
                auto dbg_surf = new QuadSurface(points_hr(used_area_hr), {1/src_step,1/src_step});
                dbg_surf->meta.extras["vc_grow_seg_from_segments_params"] = vc::tracer::toJson(params).dump();
        ...
        if (update_mapping) {
            ...
            optimize_surface_mapping(opt_data, opt_state, opt_points, opt_generations, active, static_bounds, step, src_step,
                                     ...
                                     /*hr_attach_lr_radius=*/params.hr_attach_lr_radius,
                                     /*hr_attach_relax_factor=*/params.hr_attach_relax_factor,
                                     /*hr_gen_parallel=*/params.hr_gen_parallel,
                                     /*remap_parallel=*/params.remap_parallel);
            if (active.area() > 0) {
    ...
    std::cout << "area exact: " << area_final_vox2 << " vx^2 (" << area_final_cm2 << " cm^2)" << "\n";
    ...
    std::cout << "pointTo total time: " << pointTo_total_ms << " ms" << "\n";
