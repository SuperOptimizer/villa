    ...
    ioPool_.setCompletionCallback(
        [this](const ChunkKey& key, std::vector<uint8_t>&& compressed) {
            if (compressed.empty()) {
                ...
                // Entries are small (16 bytes key + set overhead), bounded by
                // total chunks in the volume, and persisted with a 7-day TTL.
                {
...
ChunkDataPtr TieredChunkCache::getBlocking(const ChunkKey& key)
{
    ...
    // Re-check hot after acquiring lock
    hot = hotGet(key);
    if (hot) return hot;
    ...
    int level, int iz0, int iy0, int ix0, int iz1, int iy1, int ix1)
{
    ...
    // Phase 1: Filter out hot-cached keys
    ...
    // Phase 2: Filter out warm-cached keys
    ...
    // Phase 3: Filter out negative-cached keys
...
void TieredChunkCache::hotEvictIfNeeded()
{
    ...
    size_t target = config_.hotMaxBytes * 15 / 16;
...
void TieredChunkCache::warmEvictIfNeeded()
{
    ...
    // Serialize eviction attempts to avoid redundant work
    std::lock_guard evictLock(warmEvictMutex_);
