...
// Resolve a .vca archive file from the volume path: either the path itself ends
// in .vca, or a directory containing exactly one .vca. Returns empty if none.
static std::filesystem::path findVcaArchive(const std::filesystem::path& path)
{
    std::error_code ec;
    if (std::filesystem::is_regular_file(path, ec) && path.extension() == ".vca")
        return path;
    if (std::filesystem::is_directory(path, ec)) {
        for (const auto& e : std::filesystem::directory_iterator(path, ec)) {
            if (e.is_regular_file() && e.path().extension() == ".vca")
                return e.path();
        }
    }
    return {};
...
void Volume::loadMetadata()
{
    ...
    // A local volume-compressor (.vca) archive carries its own geometry; there is
    // no meta.json. Synthesize minimal metadata and tag the format so zarrOpen()
    // is skipped and vcaOpen() populates level shapes from the archive instead.
    vcaPath_ = findVcaArchive(path_);
    if (!vcaPath_.empty()) {
        const auto baseName = path_.filename().string();
        metadata_["uuid"] = baseName;
        metadata_["name"] = baseName;
        metadata_["type"] = "vol";
        metadata_["format"] = "vca";
        metadata_["width"] = 0;   // filled by vcaOpen() from LOD0 dims
        metadata_["height"] = 0;
        metadata_["slices"] = 0;
...
void Volume::vcaOpen()
{
    // Populate the per-level shape/chunk/dtype metadata (shared with the zarr
    // path — the zarrLevel*_ members are generic level info) from the .vca
    // archive's LOD pyramid, and set the base dimensions from LOD0.
    auto opened = vc::render::openVcaArchive(vcaPath_);
    if (opened.shapes.empty()) {
        throw std::runtime_error("no LODs in vca archive " + vcaPath_.string());
    }
    zarrLevelShapes_ = opened.shapes;
    zarrLevelChunkShapes_ = opened.chunkShapes;
    zarrLevelStorageChunkShapes_ = opened.storageChunkShapes;
    ...
    const auto& lod0 = zarrLevelShapes_.front(); // [z,y,x]
    _slices = lod0[0];
    _height = lod0[1];
    _width = lod0[2];
    metadata_["slices"] = _slices;
