...
template<typename T>
static void readArea3DImpl(utils::Tensor& out, const cv::Vec3i& offset, vc::cache::TieredChunkCache* cache, int level) {

    CacheParams p(cache, level);

    // out is pre-allocated with shape {dimZ, dimY, dimX} in column-major (Order::F) layout
    auto shape = out.shape();
    cv::Vec3i size = {(int)shape[0], (int)shape[1], (int)shape[2]};
    cv::Vec3i to = offset + size;

    T* outPtr = out.data<T>();
    auto strides = out.strides();
    // strides are in bytes for utils::Tensor
    const ptrdiff_t strideZ = strides[0] / static_cast<ptrdiff_t>(sizeof(T));
    const ptrdiff_t strideY = strides[1] / static_cast<ptrdiff_t>(sizeof(T));
    const ptrdiff_t strideX = strides[2] / static_cast<ptrdiff_t>(sizeof(T));
    ...
    for (const auto& idx : chunks_to_process) {
        ...
        if (chunkPtr) {
            ...
            int chunkStrideZ = p.cy * p.cx;
            int chunkStrideY = p.cx;
            for (int z = copy_from_start[0]; z < copy_from_end[0]; ++z) {
                for (int y = copy_from_start[1]; y < copy_from_end[1]; ++y) {
                    for (int x = copy_from_start[2]; x < copy_from_end[2]; ++x) {
                        ...
                        int oy = y - offset[1];
                        int ox = x - offset[2];
                        outPtr[oz * strideZ + oy * strideY + ox * strideX] = chunkData[lz * chunkStrideZ + ly * chunkStrideY + lx];
                    }
                }
            }
        } else {
            for (int z = copy_from_start[0]; z < copy_from_end[0]; ++z) {
                for (int y = copy_from_start[1]; y < copy_from_end[1]; ++y) {
                    for (int x = copy_from_start[2]; x < copy_from_end[2]; ++x) {
                        ...
                        int oy = y - offset[1];
                        int ox = x - offset[2];
                        outPtr[oz * strideZ + oy * strideY + ox * strideX] = 0;
                    }
                }
            }
        }
...
void readArea3D(utils::Tensor& out, const cv::Vec3i& offset, vc::cache::TieredChunkCache* cache, int level) {
    switch (out.dtype()) {
        case utils::DType::UInt8:
            readArea3DImpl<uint8_t>(out, offset, cache, level);
            break;
        case utils::DType::UInt16:
            readArea3DImpl<uint16_t>(out, offset, cache, level);
            break;
        default:
            break;
    }
    ...
    float dsScale)
{
    ...
    std::vector<uint8_t> localVolume(localD * localH * localW);
    ...
    // Row-major strides for [D, H, W] layout
    const size_t volStrideZ = localH * localW;
    const size_t volStrideY = localW;
    ...
    auto sampleLocal = [&](float gx, float gy, float gz) -> float {
        ...
        return static_cast<float>(localVolume[static_cast<size_t>(lz) * volStrideZ + static_cast<size_t>(ly) * volStrideY + static_cast<size_t>(lx)]);
    };
