...
static float tdist_sum(const cv::Vec3f& point, const std::vector<cv::Vec3f>& tgts, const std::vector<float>& tds)
{
    float sum = 0;
    for (size_t i = 0; i < tgts.size(); i++) {
        float dist = cv::norm(point - tgts[i]);
        float target_dist = i < tds.size() ? tds[i] : 0;
