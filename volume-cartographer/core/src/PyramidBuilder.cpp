...
template <typename T>
void writeZarrBand(zarr::Zarr* dsOut, const std::vector<cv::Mat>& slices,
...
template void writeZarrBand<uint8_t>(zarr::Zarr*, const std::vector<cv::Mat>&,
    uint32_t, const std::vector<size_t>&, size_t, size_t, int, int);
template void writeZarrBand<uint16_t>(zarr::Zarr*, const std::vector<cv::Mat>&,
    uint32_t, const std::vector<size_t>&, size_t, size_t, int, int);
...
template <typename T>
void buildPyramidLevel(const std::filesystem::path& outRoot, int level,
                       size_t CH, size_t CW,
                       int numParts, int partId)
{
    auto src = zarr::openDataset(outRoot, std::to_string(level - 1));
    auto dst = zarr::openDataset(outRoot, std::to_string(level));
    const auto& ss = src->shape();
    const auto& sc = src->chunkShape();
    const auto& dc = dst->chunkShape();
...
void createPyramidDatasets(const std::filesystem::path& outRoot,
                           ...
                           size_t CH, size_t CW, bool isU16)
{
    ...
    for (int level = 1; level <= 5; level++) {
        ...
        size_t chZ = std::min(ds[0], shape0[0]);
        std::vector<size_t> dc = {chZ, std::min(CH, ds[1]), std::min(CW, ds[2])};
        zarr::createArray(outRoot, std::to_string(level), dtype, ds, dc, std::string("blosc"), compOpts);
        prevShape = ds;
    }
}
...
void writeZarrAttrs(const std::filesystem::path& outRoot,
                    ...
                    const cv::Size& canvasSize, size_t CZ, size_t CH, size_t CW)
{
    ...
    zarr::writeAttributes(outRoot, attrs);
}
