...
{
    if (!ctx.dataset || ctx.chunk_size <= 0) {
        return nullptr;
    }
    ...
    // binary_data is row-major (z,y,x): binary_data[z*cs*cs + y*cs + x]
    // This matches sample_sdt()'s access pattern: data[z*sy*sx + y*sx + x]
    std::vector<uint8_t> binary_data(static_cast<size_t>(cs) * cs * cs, 0);

    auto ds_shape = ctx.dataset->shape(); // z,y,x
    // origin is in xyz order; ds_shape is in zyx order
    cv::Vec3i clamped_origin(
        std::max(0, origin[0]),
        std::max(0, origin[1]),
        std::max(0, origin[2]));
    cv::Vec3i clamped_end(
        std::min(static_cast<int>(ds_shape[2]), origin[0] + cs),
        std::min(static_cast<int>(ds_shape[1]), origin[1] + cs),
        std::min(static_cast<int>(ds_shape[0]), origin[2] + cs));
    cv::Vec3i read_size = clamped_end - clamped_origin;

    if (read_size[0] > 0 && read_size[1] > 0 && read_size[2] > 0) {
        // Convert to ZYX for readRegion
        std::vector<size_t> read_offset_zyx = {
            static_cast<size_t>(clamped_origin[2]),
            static_cast<size_t>(clamped_origin[1]),
            static_cast<size_t>(clamped_origin[0])};
        std::vector<size_t> read_shape_zyx = {
            static_cast<size_t>(read_size[2]),
            static_cast<size_t>(read_size[1]),
            static_cast<size_t>(read_size[0])};
        std::vector<uint8_t> read_buf(read_shape_zyx[0] * read_shape_zyx[1] * read_shape_zyx[2]);
        ctx.dataset->readRegion(read_offset_zyx, read_shape_zyx, read_buf.data());

        const cv::Vec3i offset = clamped_origin - origin;
        // read_buf is C-order ZYX: buf[z*ny*nx + y*nx + x]
        const size_t rny = read_shape_zyx[1], rnx = read_shape_zyx[2];
        for (int z = 0; z < read_size[2]; ++z) {
            for (int y = 0; y < read_size[1]; ++y) {
                for (int x = 0; x < read_size[0]; ++x) {
                    const uint8_t v = read_buf[z * rny * rnx + y * rnx + x];
                    // Store in binary_data as row-major (z,y,x)
                    const size_t dst = static_cast<size_t>(z + offset[2]) * cs * cs +
                                       static_cast<size_t>(y + offset[1]) * cs +
                                       static_cast<size_t>(x + offset[0]);
                    binary_data[dst] = static_cast<uint8_t>(v >= ctx.threshold ? 1 : 0);
                }
            }
        }
    }
    ...
    for (size_t i = 0; i < voxels; ++i) {
        chunk.data[i] = binary_data[i] ? -edt_inside[i] : edt_outside[i];
    }
...
#pragma omp parallel for
        ...
        small = outer.sub(low, high, low, high, low, high);
    }

};


QuadSurface *tracer(vc::Zarr *ds, float scale, cv::Vec3f origin, const nlohmann::json &params, const std::string &cache_root, float voxelsize, std::vector<DirectionField> const &direction_fields, QuadSurface* resume_surf, const std::filesystem::path& tgt_path, const nlohmann::json& meta_params, const VCCollection &corrections)
{
    ...
    if (params.contains("normal3d_zarr_path") && params["normal3d_zarr_path"].is_string()) {
        try {
            ...
            try {
                auto attrs = vc::Zarr::readAttributes(zarr_root);
            ...
            const int vol_z = static_cast<int>(vol_shape_zyx[0]);
            const int vol_y = static_cast<int>(vol_shape_zyx[1]);
            const int vol_x = static_cast<int>(vol_shape_zyx[2]);

            auto x_ds = std::make_unique<vc::Zarr>(vc::Zarr::open(zarr_root / "x" / std::to_string(scale_level)));
            const auto& nshape = x_ds->shape();
            if (nshape.size() != 3) {
                throw std::runtime_error("normal3d x/0 dataset is not 3D");
            }
            const int nz = static_cast<int>(nshape[0]);
            const int ny = static_cast<int>(nshape[1]);
            const int nx = static_cast<int>(nshape[2]);
            if (nz <= 0 || ny <= 0 || nx <= 0) {
            ...
            std::vector<std::unique_ptr<vc::Zarr>> dss;
            for (auto dim : std::string("xyz")) {
                dss.push_back(std::make_unique<vc::Zarr>(vc::Zarr::open(zarr_root / std::string(&dim, 1) / std::to_string(scale_level))));
            }
            ...
            try {
                auto ds_rms = std::make_unique<vc::Zarr>(vc::Zarr::open(zarr_root / "fit_rms" / std::to_string(scale_level)));
                auto ds_frac = std::make_unique<vc::Zarr>(vc::Zarr::open(zarr_root / "fit_frac_short_paths" / std::to_string(scale_level)));
    ...
    Chunked3d<uint8_t,thresholdedDistance> proc_tensor(compute, ds, cache_root);
    ...
    if (loss_settings.w[LossType::SPACELINE] > 0.0f && loss_settings.space_line_steps >= 2) {
        ...
        trace_data.space_line_volume = std::make_unique<Chunked3d<uint8_t, lineLossDistance>>(
            *trace_data.space_line_compute, ds, cache_root);
    ...
    Chunked3d<uint8_t,passTroughComputor> dbg_tensor(pass, ds);
