    ...
    const CompositeParams* params)
{
    ...
    // Phase 1: Discover needed chunks and prefetch (single-threaded).
    {
        ...
        // When all chunks are already cached, get() is a trivial atomic load
        // and OMP overhead (thread wake/sync) dominates.
        bool anyMissing = false;
        for (size_t ci = 0; ci < neededChunks.size(); ci++) {
            if (!cache.getIfCached(neededChunks[ci][0], neededChunks[ci][1], neededChunks[ci][2])) {
                anyMissing = true;
                break;
            }
        }
