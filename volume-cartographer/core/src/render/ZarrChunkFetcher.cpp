...
// Re-express an opened pyramid through a matter-compressor (.mca) cache at `mcaPath`:
// wrap each level's source fetcher in a MatterCacheFetcher sharing one persistent .mca,
                      ...
                      std::vector<ChunkCache::LevelInfo>& levelInfoOut)
{
    ...
    levelInfoOut.clear();
    levelInfoOut.reserve(opened.shapes.size());
    for (std::size_t i = 0; i < opened.shapes.size(); ++i) {
        ...
        levelInfoOut.push_back({opened.shapes[i],
