...
void to_json(Json& j, const Collection& c) {
    ...
    if (c.autoFillMode != WindingFillMode::None) {
...
void from_json(const Json& j, Collection& c) {
    ...
    if (j.contains("autoFillMode") && j.at("autoFillMode").is_number()) {
        ...
        if (modeInt >= 0 && modeInt <= 3) {
            c.autoFillMode = static_cast<WindingFillMode>(modeInt);
        }
    }
...
WindingFillMode PointCollections::getAutoFillMode(uint64_t collectionId) const
{
    if (_collections.count(collectionId)) {
...
bool PointCollections::loadFromJSON(const std::string& filename)
{
    ...
    try {
        ...
        std::cerr << "Failed to extract data from JSON: " << e.what();
        return false;
    }
...
bool PointCollections::saveToSegmentPath(const std::filesystem::path& segmentPath) const
{
    ...
    if (anchoredCount == 0) {
        ...
        if (std::filesystem::exists(filePath, ec)) {
            ...
            std::cerr << "Removed empty corrections file:" << filePath.string();
        }
        return true;
    }
    ...
    if (!o.is_open()) {
        std::cerr << "Failed to open corrections file for writing:" << filePath.string();
        return false;
    }
    o << j.dump(4);
    o.close();

    std::cerr << "Saved" << anchoredCount << "anchored correction collections to" << filePath.string();
    return true;
}
...
void PointCollections::applyAnchorOffset(float offsetX, float offsetY)
{
    ...
    if (updatedCount > 0) {
        std::cerr << "Applied offset (" << offsetX << "," << offsetY << ") to" << updatedCount << "correction anchors";
    }
}
...
bool PointCollections::loadFromSegmentPath(const std::filesystem::path& segmentPath)
{
    ...
    if (!toRemove.empty()) {
        std::cerr << "Clearing" << toRemove.size() << "existing anchored correction collections";
    }
    ...
    if (!std::filesystem::exists(filePath, ec)) {
        ...
        std::cerr << "No corrections file at" << filePath.string();
        return true;
    }
    ...
    try {
        ...
        std::cerr << "Failed to parse corrections JSON:" << e.what();
        return false;
    }
    ...
    try {
        if (!j.contains("vc_pointcollections_json_version") ||
            ...
            std::cerr << "Corrections file has incorrect version";
            return false;
        }
        ...
        std::cerr << "Loaded" << loadedCount << "anchored correction collections from" << filePath.string();

    } catch (const std::exception& e) {
        std::cerr << "Failed to extract data from corrections JSON:" << e.what();
        return false;
    }
