...
/**
  ...
  A Dictionary defines a set of metadata keys and the associated types
  of the corresponding values (e.g. the key "version" should be interpreted as
  an `int`). A Library holds several such dictionaries: one dictionary
  for each unique version of VolumePkg.

  VERSION_LIBRARY holds the metadata keys and expected value-types for
