...
struct passTroughComputor
{
    ...
    template <typename T, typename E> void compute(const T &large, T &small, const cv::Vec3i &offset_large)
    {
        ...
        small = vc::view(large, {vc::range(low,high),vc::range(low,high),vc::range(low,high)});
    }
};
...
class Chunked3d {
public:
    using CHUNKT = vc::Tensor<T>;

    Chunked3d(C &compute_f, zarr::Zarr *ds, ChunkCache<T> *cache) : _compute_f(compute_f), _ds(ds), _cache(cache)
    {
    ...
    Chunked3d(C &compute_f, zarr::Zarr *ds, ChunkCache<T> *cache, const std::filesystem::path &cache_root) : _compute_f(compute_f), _ds(ds), _cache(cache)
    {
        _border = compute_f.BORDER;
...
#pragma omp atomic
        ...
        CHUNKT small = vc::empty<T>({(size_t)s,(size_t)s,(size_t)s}, vc::Layout::ColumnMajor);
        CHUNKT large;
        if (_ds) {
            large = vc::empty<T>({(size_t)(s+2*_border),(size_t)(s+2*_border),(size_t)(s+2*_border)}, vc::Layout::ColumnMajor);
            large.fill(C::FILL_V);
...
{
    Chunked3dFloatFromUint8(std::unique_ptr<zarr::Zarr> &&ds, float scale, ChunkCache<uint8_t> *cache, std::string const &cache_root, std::string const &unique_id) :
    ...
    float _scale;
    std::unique_ptr<zarr::Zarr> _ds;
};

struct Chunked3dVec3fFromUint8
{
    Chunked3dVec3fFromUint8(std::vector<std::unique_ptr<zarr::Zarr>> &&dss, float scale, ChunkCache<uint8_t> *cache, std::string const &cache_root, std::string const &unique_id) :
    ...
    float _scale;
    std::vector<std::unique_ptr<zarr::Zarr>> _dss;
};
