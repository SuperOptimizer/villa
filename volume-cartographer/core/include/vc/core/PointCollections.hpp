...
struct Collection
{
    ...
    cv::Vec3f color;
    std::optional<cv::Vec2f> anchor2d;
    WindingFillMode autoFillMode = WindingFillMode::None;
    float autoFillConstant = 0.0f;
    std::unordered_map<std::string, std::string> tags;
};

// Plain (Qt-free) collection of named ColPoint sets. Hooks (on*) are no-ops
// here; VCCollection in vc_ui overrides them to emit Qt signals.
...
public:
    ...
    const std::unordered_map<uint64_t, Collection>& getAllCollections() const { return _collections; }
...
protected:
    // Notification hooks. Default impls are empty. VCCollection overrides
    // to emit Qt signals.
...
void to_json(utils::Json& j, const Collection& c);
void from_json(const utils::Json& j, Collection& c);
