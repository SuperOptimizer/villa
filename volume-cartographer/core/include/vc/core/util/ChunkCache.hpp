 ...
 *
 * Supports caching chunks from multiple zarr::Zarr instances simultaneously.
...
{
public:
    using ChunkPtr = std::shared_ptr<vc::Tensor<T>>;
     ...
     */
    ChunkPtr get(zarr::Zarr* ds, int iz, int iy, int ix);

    /**
     * @brief Check if chunk is cached without loading.
     */
    ChunkPtr getIfCached(zarr::Zarr* ds, int iz, int iy, int ix) const;

    void prefetch(zarr::Zarr* ds, int minIz, int minIy, int minIx, int maxIz, int maxIy, int maxIx);
    void clear();
    void flush();
...
private:
    ...
    ChunkPtr loadChunk(zarr::Zarr* ds, int iz, int iy, int ix);
    void evictIfNeeded();
