...
#define NB_BANDS 8       // significance contexts by L1-frequency band
#define MAGCTX   12      // unary magnitude contexts
typedef struct { ctx_t sig[NB_BANDS]; ctx_t mag[MAGCTX]; } atom_ctx;
...
static int eob_bits_S(int n){ int b=0; while((1<<b) <= n) b++; return b; }
...
static void enc_block_coefs(rc_enc*e,const rc_i16*q,int S){
    ...
    for(rc_u32 p=0;p<eob;++p){ rc_u32 idx=scan[p]; int b=band_of_S(idx,S); rc_i16 v=q[idx];
        ...
        enc_bit(e,&ac.sig[b],1); rc_u32 m=(rc_u32)(v<0?-v:v); enc_magnitude(e,&ac,m); enc_bypass(e,v<0?1:0);
    }
}
static void dec_block_coefs(rc_dec*d,rc_i16*q,int S){
    ...
    for(rc_u32 p=0;p<eob;++p){ rc_u32 idx=scan[p]; int b=band_of_S(idx,S);
        if(!dec_bit(d,&ac.sig[b]))continue; rc_u32 m=dec_magnitude(d,&ac); int neg=dec_bypass(d);
