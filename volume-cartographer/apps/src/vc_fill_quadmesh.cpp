    ...
    for (auto [j, i, q00, q01, q10, q11] : ValidQuadRange<cv::Vec3f>(&points_tgt)) {
            ...
            for(int sy=0;sy<=step;sy++)
                for(int sx=0;sx<=step;sx++) {
                    const float fx = float(sx)/step;
                    const float fy = float(sy)/step;
                ...
                if (!counts_hr(j*step+1,i*step+1)) {
                    for(int sy=0;sy<=step;sy++)
                        for(int sx=0;sx<=step;sx++) {
                            ...
                            const float fy = float(sy)/step;
                            // FMA-friendly lerp: a + t*(b-a)
                            const cv::Vec3f c0 = points_tgt(j,i) + fx * (points_tgt(j,i+1) - points_tgt(j,i));
                            const cv::Vec3f c1 = points_tgt(j+1,i) + fx * (points_tgt(j+1,i+1) - points_tgt(j+1,i));
                            const cv::Vec3f c = c0 + fy * (c1 - c0);
                            // std::cout << l << at_int(points_src, {l[1],l[0]}) << "\n";
...
#pragma omp critical
                    ...
                    if (supports[s](p)) {
                        if (loc_valid(surf_points[s], surf_locs[s](p))) {
                            if (std::abs(at_int(winds[s], {static_cast<float>(surf_locs[s](p)[1]),static_cast<float>(surf_locs[s](p)[0])}) - tgt_wind[x]) <= wind_th) {
                                ...
                                std::cout << "wind th " << std::abs(at_int(winds[s], {static_cast<float>(surf_locs[s](p)[1]),static_cast<float>(surf_locs[s](p)[0])}) - tgt_wind[x]) << " " << at_int(winds[s], {static_cast<float>(surf_locs[s](p)[1]),static_cast<float>(surf_locs[s](p)[0])}) << tgt_wind[x] << " " << "\n";
                        }
                        else
                        {
    ...
    
    {
        ...
        surf_full->meta.extras["vc_fill_quadmesh_params"] = params.dump();
