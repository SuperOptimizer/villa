...
static void run_vis_normals_zarr_as_ply(const fs::path& zarr_root, const fs::path& out_ply, const std::optional<CropBox3i>& crop_opt) {
    ...
    auto open_u8_zyx = [&](const char* axis) -> std::unique_ptr<zarr::Zarr> {
        return zarr::openDataset(zarr_root / axis, "0");
    };
    ...
    vc::Tensor<uint8_t> ax = vc::zeros<uint8_t>({Z, Y, X});
    vc::Tensor<uint8_t> ay = vc::zeros<uint8_t>({Z, Y, X});
    vc::Tensor<uint8_t> az = vc::zeros<uint8_t>({Z, Y, X});
    zarr::Shape off = {0, 0, 0};
    zarr::readSubarray<uint8_t>(*dsx, ax, off);
    zarr::readSubarray<uint8_t>(*dsy, ay, off);
    zarr::readSubarray<uint8_t>(*dsz, az, off);
...
static void run_vis_normals_zarr_on_surf_edges_as_ply(
    ...
    auto sample_trilinear = [&](const cv::Point3f& p_xyz, cv::Point3f& out_n_xyz) -> bool {
        ...
        auto c = [&](const vc::Tensor<uint8_t>& a, int zz, int yy, int xx) -> double {
...
static void run_align_normals_zarr(
    ...
    vc::Tensor<uint8_t> ax = vc::zeros<uint8_t>({CZ, CY, CX});
    vc::Tensor<uint8_t> ay = vc::zeros<uint8_t>({CZ, CY, CX});
    vc::Tensor<uint8_t> az = vc::zeros<uint8_t>({CZ, CY, CX});
    zarr::readSubarray<uint8_t>(*dsx, ax, crop_zyx.off);
    zarr::readSubarray<uint8_t>(*dsy, ay, crop_zyx.off);
    zarr::readSubarray<uint8_t>(*dsz, az, crop_zyx.off);
    ...
    zarr::createStore(out_zarr);
    zarr::createGroup(out_zarr / "x");
    zarr::createGroup(out_zarr / "y");
    zarr::createGroup(out_zarr / "z");
    ...
    auto compCfg = zarr::CompressorConfig::blosc("zstd", 1, 0);

    auto out_dsx = zarr::createArray(out_zarr / "x", "0", zarr::Dtype::uint8, full_shape, chunks, compCfg, /*fillValue=*/128, "/");
    auto out_dsy = zarr::createArray(out_zarr / "y", "0", zarr::Dtype::uint8, full_shape, chunks, compCfg, /*fillValue=*/128, "/");
    auto out_dsz = zarr::createArray(out_zarr / "z", "0", zarr::Dtype::uint8, full_shape, chunks, compCfg, /*fillValue=*/128, "/");

    zarr::writeSubarray<uint8_t>(*out_dsx, ax, crop_zyx.off);
    zarr::writeSubarray<uint8_t>(*out_dsy, ay, crop_zyx.off);
    zarr::writeSubarray<uint8_t>(*out_dsz, az, crop_zyx.off);
    ...
    zarr::writeAttributes(out_zarr, attrs);
}
...
static void run_fit_normals(
    ...
    if (out_zarr_opt.has_value()) {
        ...
        // Ensure groups exist for creating datasets.
        zarr::createGroup(out_zarr / "x");
        zarr::createGroup(out_zarr / "y");
        zarr::createGroup(out_zarr / "z");
        zarr::createGroup(out_zarr / "fit_rms");
        zarr::createGroup(out_zarr / "fit_frac_short_paths");
        zarr::createGroup(out_zarr / "fit_used_radius");
        zarr::createGroup(out_zarr / "fit_segment_count");
        ...
        auto dsx = zarr::createArray(out_zarr / "x", "0", zarr::Dtype::uint8, shape, chunks, compCfg, /*fillValue=*/128, "/");
        auto dsy = zarr::createArray(out_zarr / "y", "0", zarr::Dtype::uint8, shape, chunks, compCfg, /*fillValue=*/128, "/");
        auto dsz = zarr::createArray(out_zarr / "z", "0", zarr::Dtype::uint8, shape, chunks, compCfg, /*fillValue=*/128, "/");
        auto ds_fit_rms = zarr::createArray(out_zarr / "fit_rms", "0", zarr::Dtype::uint8, shape, chunks, compCfg, /*fillValue=*/0, "/");
        auto ds_fit_frac = zarr::createArray(out_zarr / "fit_frac_short_paths", "0", zarr::Dtype::uint8, shape, chunks, compCfg, /*fillValue=*/0, "/");
        auto ds_fit_rad = zarr::createArray(out_zarr / "fit_used_radius", "0", zarr::Dtype::uint8, shape, chunks, compCfg, /*fillValue=*/0, "/");
        auto ds_fit_sc = zarr::createArray(out_zarr / "fit_segment_count", "0", zarr::Dtype::uint8, shape, chunks, compCfg, /*fillValue=*/0, "/");

        auto ax = vc::adapt(enc_x.data(), shape);
        auto ay = vc::adapt(enc_y.data(), shape);
        auto az = vc::adapt(enc_z.data(), shape);
        auto a_fit_rms = vc::adapt(enc_fit_rms.data(), shape);
        auto a_fit_frac = vc::adapt(enc_fit_frac_short_paths.data(), shape);
        auto a_fit_rad = vc::adapt(enc_fit_used_radius.data(), shape);
        auto a_fit_sc = vc::adapt(enc_fit_segment_count.data(), shape);
        zarr::Shape off = {0, 0, 0};
        zarr::writeSubarray<uint8_t>(*dsx, ax, off);
        zarr::writeSubarray<uint8_t>(*dsy, ay, off);
        zarr::writeSubarray<uint8_t>(*dsz, az, off);
        zarr::writeSubarray<uint8_t>(*ds_fit_rms, a_fit_rms, off);
        zarr::writeSubarray<uint8_t>(*ds_fit_frac, a_fit_frac, off);
        zarr::writeSubarray<uint8_t>(*ds_fit_rad, a_fit_rad, off);
        zarr::writeSubarray<uint8_t>(*ds_fit_sc, a_fit_sc, off);
