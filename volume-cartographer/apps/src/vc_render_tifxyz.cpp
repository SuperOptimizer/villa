...
static void renderBands(
    QuadSurface* surf, vc::cache::TieredChunkCache* cache,
    ...
    for (uint32_t bi = bandStart; bi < bandEnd; bi++) {
        ...
        if (isComposite) {
            ...
            if constexpr (std::is_same_v<T, uint8_t>) {
                readCompositeFast(compOut, cache, 0,
                                  base, dirs,
            ...
            readMultiSlice(raw, cache, 0, base, dirs, allOffsets);
        ...
        if (since >= 1.0 || done == bandsThis) {
            ...
            auto cs = cache->stats();
            uint64_t totalHits = cs.hotHits + cs.warmHits + cs.coldHits;
            uint64_t tot = totalHits + cs.iceFetches + cs.misses;
            double hr = tot > 0 ? 100.0 * totalHits / tot : 0.0;
            double gbR = cs.hotBytes / (1024.0*1024.0*1024.0);
            const char* prefix = g_logFile ? "  " : "\r  ";
            const char* suffix = g_logFile ? "\n" : "";
            logPrintf(stderr, "%sband %u/%u (%d%%)  %.1f bands/s  %dm%02ds  eta %dm%02ds  cache %.1f%% evict %lu hotMB %.0f%s",
                prefix, done, bandsThis, int(100.0 * done / bandsThis),
                bandsPerSec,
                int(elapsed)/60, int(elapsed)%60, int(eta)/60, int(eta)%60,
                hr, (unsigned long)cs.hotEvictions, cs.hotBytes/(1024.0*1024.0), suffix);
        }
    }
    ...
    for (uint32_t ty = tyStart; ty < tyEnd; ty++) {
        ...
        for (uint32_t tx = 0; tx < numTileCols; tx++) {
            ...
            if (isComposite) {
                ...
                sampleTileSlices(raw, cache, 0, base, dirs, allOffsets);
            }
        ...
        if (since >= 1.0 || done == tileRowsThis) {
            ...
            const char* prefix = g_logFile ? "  " : "\r  ";
            const char* suffix = g_logFile ? "\n" : "";
            logPrintf(stderr, "%stile-row %u/%u (%d%%)  %.1f chunks/s  %dm%02ds  eta %dm%02ds  cache %.1f%% evict %lu hotMB %.0f%s",
...
int main(int argc, char *argv[])
{
    ...
    // --- Open source volume ---
    auto ds = std::make_unique<vc::VcDataset>(vol_path / std::to_string(group_idx));
    {
    ...
    {
        std::ostringstream oss; oss << ds->defaultChunkShape();
    ...
    auto tieredCache = vc::cache::createSimpleTieredCache(ds.get(), cache_bytes, vol_path / std::to_string(group_idx));
    ...
    auto process_one = [&](const std::filesystem::path& seg_folder) {
        ...
        std::filesystem::path outFile(wantZarr ? zarrOutputArg : "/dev/null");
        ...
        if (wantZarr) {
            ...
            outFile = std::filesystem::path(zarrOutputArg);
            std::vector<size_t> shape0 = {baseZ, baseY, baseX};
            chunks0 = {shape0[0], std::min(CH, shape0[1]), std::min(CW, shape0[2])};
            vc::VcDtype vcDtype = useU16 ? vc::VcDtype::uint16 : vc::VcDtype::uint8;

            if (pre_flag) {
                logPrintf(stdout, "[pre] creating zarr + all levels...\n");
                vc::createZarrDataset(outFile, "0", shape0, chunks0, vcDtype, "zstd");
                ...
                dsOut = std::make_unique<vc::VcDataset>(outFile / "0");
                ...
                dsOut = vc::createZarrDataset(outFile, "0", shape0, chunks0, vcDtype, "zstd");
            }
        ...
        if (wantZarr && wantPyramid && !pre_flag) {
            ...
            if (inlinePyramid) {
                for (int level = 1; level <= 5; level++) {
                    pyramidOwned.push_back(std::make_unique<vc::VcDataset>(outFile / std::to_string(level)));
        ...
        // ---- Render pass ----
        {
            if (wantZarr) {
                ...
                if (useU16)
                    renderTiles<uint16_t>(surf.get(), tieredCache.get(),
                    ...
                    renderTiles<uint8_t>(surf.get(), tieredCache.get(),
                        full_size, crop, tgt_size, float(render_scale), scale_seg, ds_scale,
                        hasAffine, affineTransform, num_slices, slice_step,
                        accumOffsets, accumType, isCompositeMode, compositeStart, compositeEnd,
                        compositeParams, rotQuad, flip_axis, numParts, partId, cvType,
                ...
                if (useU16)
                    renderBands<uint16_t>(surf.get(), tieredCache.get(),
                        full_size, crop, tgt_size, float(render_scale), scale_seg, ds_scale,
                        hasAffine, affineTransform, num_slices, slice_step,
                        accumOffsets, accumType, isCompositeMode, compositeStart, compositeEnd,
                        compositeParams, rotQuad, flip_axis, numParts, partId, cvType, bandH, writerFn);
                else
                    renderBands<uint8_t>(surf.get(), tieredCache.get(),
                        full_size, crop, tgt_size, float(render_scale), scale_seg, ds_scale,
                        hasAffine, affineTransform, num_slices, slice_step,
                        accumOffsets, accumType, isCompositeMode, compositeStart, compositeEnd,
                        compositeParams, rotQuad, flip_axis, numParts, partId, cvType, bandH, writerFn);
    ...
    {
        auto s = tieredCache->stats();
        uint64_t totalHits = s.hotHits + s.warmHits + s.coldHits;
        uint64_t tot = totalHits + s.iceFetches + s.misses;
        if (tot > 0) {
            logPrintf(stdout, "[tiered cache] hot=%lu warm=%lu cold=%lu ice=%lu miss=%lu rate=%.1f%% "
                      "hotEvict=%lu warmEvict=%lu hotMB=%.1f warmMB=%.1f\n",
                      (unsigned long)s.hotHits, (unsigned long)s.warmHits,
                      (unsigned long)s.coldHits, (unsigned long)s.iceFetches,
                      (unsigned long)s.misses,
                      tot > 0 ? 100.0*totalHits/tot : 0.0,
                      (unsigned long)s.hotEvictions, (unsigned long)s.warmEvictions,
                      s.hotBytes/(1024.0*1024.0), s.warmBytes/(1024.0*1024.0));
        }
    }
