...
int main(int argc, char** argv) {
    ...
    try {
        ...
        if (dir_mode && is_zarr_root(in_path)) {
            ...
            std::unique_ptr<zarr::Zarr> dsIn;
            if (path_is_dataset) {
                dsIn = zarr::openArray(in_root.parent_path(), in_root.filename().string());
            } else {
                dsIn = zarr::openDataset(in_root, std::to_string(group_idx));
            }
            ...
            if (!out_is_zarr) {
                ...
                for (long long zi = static_cast<long long>(z0); zi <= static_cast<long long>(z1); ++zi) {
                    ...
                    cv::Mat src;
                    // Read one Z slab
                    if (dsIn->dtype() == zarr::Dtype::uint8) {
                        vc::Tensor<uint8_t> slab = vc::empty<uint8_t>({1ul, Y, X});
                        zarr::Shape off = {z, 0ul, 0ul};
                        zarr::readSubarray(*dsIn, slab, off);
                    ...
                    } else if (dsIn->dtype() == zarr::Dtype::uint16) {
                        vc::Tensor<uint16_t> slab = vc::empty<uint16_t>({1ul, Y, X});
                        zarr::Shape off = {z, 0ul, 0ul};
                        zarr::readSubarray(*dsIn, slab, off);
                    ...
                    } else if (dsIn->dtype() == zarr::Dtype::float32) {
                        vc::Tensor<float> slab = vc::empty<float>({1ul, Y, X});
                        zarr::Shape off = {z, 0ul, 0ul};
                        zarr::readSubarray(*dsIn, slab, off);
            ...
            zarr::CompressorConfig comp0 = zarr::CompressorConfig::blosc("zstd", 1, 0);
            auto dsOut0 = zarr::createArray(out_root, "0", zarr::Dtype::uint8, shape0, chunks0, comp0);
            ...
            for (long long zi = static_cast<long long>(z0); zi <= static_cast<long long>(z1); ++zi) {
                ...
                vc::Tensor<uint8_t> slabOut = vc::empty<uint8_t>({1ul, Y, X});
                for (size_t y = 0; y < Y; ++y) {
                    const uint8_t* row = out_u8.ptr<uint8_t>(static_cast<int>(y));
                    for (size_t x = 0; x < X; ++x) slabOut(0, y, x) = row[x];
                }
                zarr::Shape offOut = {z, 0ul, 0ul};
                zarr::writeSubarray(*dsOut0, slabOut, offOut);
            ...
            zarr::writeAttributes(out_root, attrs);
            ...
            for (int targetLevel = 1; targetLevel <= 5; ++targetLevel) {
                auto srcDs = zarr::openDataset(out_root, std::to_string(targetLevel - 1));
                ...
                zarr::CompressorConfig compPyr = zarr::CompressorConfig::blosc("zstd", 1, 0);
                auto dstDs = zarr::createArray(out_root, std::to_string(targetLevel), zarr::Dtype::uint8, dShape, dChunks, compPyr);
                ...
                for (long long ty = 0; ty < static_cast<long long>(tilesY); ++ty) {
                    for (long long tx = 0; tx < static_cast<long long>(tilesX); ++tx) {
                        ...
                        vc::Tensor<uint8_t> srcChunk = vc::empty<uint8_t>({lz, ly*2ul, lx*2ul});
                        {
                            zarr::Shape off = {0ul, y0*2ul, x0*2ul};
                            zarr::readSubarray(*srcDs, srcChunk, off);
                        }
                        vc::Tensor<uint8_t> dstChunk = vc::empty<uint8_t>({lz, ly, lx});
                        ...
                        zarr::Shape offD = {0ul, y0, x0};
                        zarr::writeSubarray(*dstDs, dstChunk, offD);
                        size_t d = ++tilesDone;
                        #ifdef _OPENMP
                        if (omp_get_thread_num() == 0)
                        #endif
