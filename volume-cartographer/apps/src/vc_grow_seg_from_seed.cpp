...
static auto load_direction_fields(json const&params, ChunkCache<uint8_t> *chunk_cache, std::filesystem::path const &cache_root)
{
    ...
    if (params.contains("direction_fields")) {
        ...
        for (auto const& direction_field : params["direction_fields"]) {
            ...
            std::vector<vc::zarr::Dataset> direction_dss;
            for (auto dim : std::string("xyz")) {
                std::filesystem::path dimPath = std::filesystem::path(zarr_path) / std::string(&dim, 1);
                direction_dss.push_back(vc::zarr::openDatasetAutoSep(dimPath, std::to_string(ome_scale)));
            }
            std::cout << "direction field dataset shape " << direction_dss.front().shape() << std::endl;
            std::optional<vc::zarr::Dataset> maybe_weight_ds;
            if (direction_field.contains("weight_zarr")) {
                std::string const weight_zarr_path = direction_field["weight_zarr"];
                maybe_weight_ds = vc::zarr::openDatasetAutoSep(weight_zarr_path, std::to_string(ome_scale));
            }
...
int main(int argc, char *argv[])
{
    ...
    auto ds = vc::zarr::openDatasetAutoSep(vol_path, "0");
    ...
    Chunked3d<uint8_t,passTroughComputor> tensor(pass, &ds, &chunk_cache);
    ...
    QuadSurface *surf = tracer(&ds, 1.0, &chunk_cache, origin, params, cache_root, voxelsize, direction_fields, resume_surf.get(), seg_dir, meta_params, corrections);
