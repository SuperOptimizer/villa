...
int main(int argc, char** argv)
{
    ...
    for (size_t z = 0; z < Z; ++z) {
        ...
        if (cvType == CV_8UC1) {
            std::vector<uint8_t> slab(Y * X);
            ds.readRegion(offset, regionShape, slab.data());
            for (size_t y = 0; y < Y; ++y) {
                auto* row = slice.ptr<uint8_t>(static_cast<int>(y));
                for (size_t x = 0; x < X; ++x) row[x] = slab[y * X + x];
            }
        } else {
            std::vector<uint16_t> slab(Y * X);
            ds.readRegion(offset, regionShape, slab.data());
            for (size_t y = 0; y < Y; ++y) {
