...
SDTChunk* getOrComputeSDTChunk(SDTContext& ctx, const cv::Vec3f& worldPt) {
    ...
    vc::Tensor<uint8_t> binaryData = vc::zeros<uint8_t>({(size_t)cs, (size_t)cs, (size_t)cs}, vc::Layout::ColumnMajor);
    ...
    if (readSize[0] > 0 && readSize[1] > 0 && readSize[2] > 0) {
        ...
        vc::Tensor<uint8_t> readBuf = vc::zeros<uint8_t>(
            {(size_t)readSizeZYX[0], (size_t)readSizeZYX[1], (size_t)readSizeZYX[2]}, vc::Layout::ColumnMajor);
    ...
    if (readSize[0] > 0 && readSize[1] > 0 && readSize[2] > 0) {
        ...
        vc::Tensor<uint8_t> readBuf = vc::zeros<uint8_t>(
            {static_cast<size_t>(readSizeZYX[0]),
             ...
             static_cast<size_t>(readSizeZYX[2])}, vc::Layout::ColumnMajor);
        readArea3D(readBuf, clampedOriginZYX, ctx.binaryDataset, ctx.cache);
