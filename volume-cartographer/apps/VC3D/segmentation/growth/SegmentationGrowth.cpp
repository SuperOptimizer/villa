                          ...
                          QString& error)
{
    ...
    try {
        ...
        std::vector<vc::zarr::Dataset> datasets;
        datasets.reserve(3);
        for (char axis : std::string("xyz")) {
            std::filesystem::path axisPath = std::filesystem::path(zarrPath) / std::string(1, axis);
            datasets.push_back(vc::zarr::openDatasetAutoSep(axisPath, std::to_string(scaleLevel)));
        }
                                   ...
                                   const TracerGrowthContext& context)
{
    ...
    vc::zarr::Dataset* dataset = context.volume->zarrDataset(0);
    if (!dataset) {
