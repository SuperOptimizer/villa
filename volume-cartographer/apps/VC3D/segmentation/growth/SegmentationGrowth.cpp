                          ...
                          QString& error)
{
    ...
    try {
        ...
        std::vector<std::unique_ptr<zarr::Zarr>> datasets;
        datasets.reserve(3);
        for (char axis : std::string("xyz")) {
            datasets.push_back(zarr::openDataset(
                std::filesystem::path(zarrPath) / std::string(1, axis),
                std::to_string(scaleLevel)));
        }
                                   ...
                                   const TracerGrowthContext& context)
{
    ...
    zarr::Zarr* dataset = context.volume->zarrDataset(0);
    if (!dataset) {
