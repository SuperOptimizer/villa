...
CVolumeViewer::CVolumeViewer(CSurfaceCollection *col, ViewerManager* manager, QWidget* parent)
    ...
    , _dragged_point_id(0)
{
    // Match old per-member defaults that differ from CompositeParams defaults
...
void CVolumeViewer::updateStatusLabel()
{
    ...
    if (_compositeSettings.enabled) {
        ...
        status += QString(" | %1(%2)").arg(method).arg(_compositeSettings.layersFront + _compositeSettings.layersBehind);
    }
...
void CVolumeViewer::setCompositeRenderSettings(const CompositeRenderSettings& settings)
{
