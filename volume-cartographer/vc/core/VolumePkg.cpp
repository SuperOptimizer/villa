...
VolumePkg::VolumePkg(const std::filesystem::path& fileLocation) : rootDir_{fileLocation}
{
    ...
    if (!std::filesystem::exists(configPath)) {
        throw std::runtime_error("could not find json file '" + configPath.string() + "'");
    }
    std::ifstream jsonFile(configPath);
    if (!jsonFile) {
        throw std::runtime_error("could not open json file '" + configPath.string() + "'");
    }
    jsonFile >> config_;
    if (jsonFile.bad()) {
        throw std::runtime_error("could not read json file '" + configPath.string() + "'");
    }
...
{
    auto name = config_.at("name").get<std::string>();
    if (name != "NULL") {
        return name;
    }
...
int VolumePkg::version() const { return config_.at("version").get<int>(); }
