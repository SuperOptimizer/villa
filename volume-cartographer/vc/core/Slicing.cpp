...
void readInterpolated3D(cv::Mat_<uint8_t> &out, z5::Dataset *ds,
    ...
    if (nearest_neighbor) {
        return readNearestNeighbor<uint8_t>(out,ds,coords,cache);
    }
    return readInterpolated3DImpl<uint8_t>(out, ds, coords, cache);
}
...
void readInterpolated3D(cv::Mat_<uint16_t> &out, z5::Dataset *ds,
    ...
    if (nearest_neighbor) {
        return readNearestNeighbor<uint16_t>(out,ds,coords,cache);
    }
    return readInterpolated3DImpl<uint16_t>(out, ds, coords, cache);
}

// Geometry utility functions are now in GeometryUtils.cpp
// Using declarations are in Slicing.hpp to make them available
