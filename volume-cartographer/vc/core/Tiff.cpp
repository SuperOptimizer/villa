                               ...
                               const WriteOptions& opts)
{
    ...
    switch (opts.compression) {
        case WriteOptions::Compression::NONE:
            compression = COMPRESSION_NONE;
            break;
        case WriteOptions::Compression::LZW:
    ...
    // Predictor
    if (opts.predictor != WriteOptions::Predictor::NONE) {
        uint16_t pred = PREDICTOR_HORIZONTAL;
        if (opts.predictor == WriteOptions::Predictor::FLOATINGPOINT && img.type() == CV_32FC1) {
#ifdef PREDICTOR_FLOATINGPOINT
            pred = PREDICTOR_FLOATINGPOINT;
#endif
        }
        TIFFSetField(tf, TIFFTAG_PREDICTOR, pred);
    }
