...
// Use geometry utility functions from vc::utils namespace
...
// Helper to convert TiffWriteOptions to vc::tiff::WriteOptions
static vc::tiff::WriteOptions toWriteOptions(const TiffWriteOptions& opts)
{
    vc::tiff::WriteOptions result;
    result.forceBigTiff = opts.forceBigTiff;
    result.tileSize = opts.tileSize;

    // Convert compression
    switch (opts.compression) {
        case TiffWriteOptions::Compression::NONE:
            result.compression = vc::tiff::WriteOptions::Compression::NONE;
            break;
        case TiffWriteOptions::Compression::LZW:
            result.compression = vc::tiff::WriteOptions::Compression::LZW;
            break;
        case TiffWriteOptions::Compression::DEFLATE:
            result.compression = vc::tiff::WriteOptions::Compression::DEFLATE;
            break;
    }
    ...
    switch (opts.predictor) {
        case TiffWriteOptions::Predictor::NONE:
            result.predictor = vc::tiff::WriteOptions::Predictor::NONE;
            break;
        case TiffWriteOptions::Predictor::HORIZONTAL:
            result.predictor = vc::tiff::WriteOptions::Predictor::HORIZONTAL;
            break;
        case TiffWriteOptions::Predictor::FLOATINGPOINT:
            result.predictor = vc::tiff::WriteOptions::Predictor::FLOATINGPOINT;
            break;
    }
...
// They are available through using declarations in Slicing.hpp
...
cv::Mat QuadSurface::channel(const std::string& name, int flags)
{
    if (_channels.count(name)) {
        ...
        if (name == "mask") {
            vc::tiff::normalizeMaskChannel(channel);
        }
        ...
        if (!channel.empty() && !(flags & SURF_CHANNEL_NORESIZE)) {
            ...
            if (name == "mask") {
                vc::tiff::normalizeMaskChannel(scaled_channel);
            }
...
void QuadSurface::save(const std::string &path_, const std::string &uuid, bool force_overwrite)
{
    ...
    try {
        vc::tiff::writeFloatBigTiff(path / "x.tif", xyz[0]);
        vc::tiff::writeFloatBigTiff(path / "y.tif", xyz[1]);
        vc::tiff::writeFloatBigTiff(path / "z.tif", xyz[2]);
    ...
    for (auto const& [name, mat] : _channels) {
        if (!mat.empty()) {
            ...
            {
                try {
                    vc::tiff::writeSingleChannelBigTiff(path / (name + ".tif"), mat, toWriteOptions(_tiff_opts));
                    wrote = true;
                } catch (...) {
