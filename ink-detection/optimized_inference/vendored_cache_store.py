"""
Vendored CacheStore implementation from zarr-python dev branch.

This is a copy of the CacheStore from:
https://github.com/zarr-developers/zarr-python/blob/eb1b6e869115925f7b653e60bd542abd410a9a4e/src/zarr/experimental/cache_store.py

Vendored here to have direct access to the implementation and avoid dependency issues.
"""
...
class CacheStore(WrapperStore[Store]):
    """
    A dual-store caching implementation for Zarr stores with negative cache support.
    ...
    Additionally, this implementation caches negative lookups (missing keys) to avoid
    repeated expensive queries to the base store for keys that don't exist. Negative
    entries are stored with the `.neg.` prefix and respect the same TTL settings.
    ...
    _negative_hits: int  # Negative cache hit counter
    ...
    async def _get_try_cache(
        ...
        if maybe_fresh_result is None:
            ...
            logger.debug("_get_try_cache: storing negative entry for key %s", key)
            neg_buffer = prototype.buffer.from_bytes(b'')
    ...
    async def _get_no_cache(
        ...
        if maybe_fresh_result is None:
            ...
            # Store negative entry
            neg_key = self._negative_key(key)
            logger.debug("_get_no_cache: storing negative entry for key %s", key)
            neg_buffer = prototype.buffer.from_bytes(b'')
            await self._cache.set(neg_key, neg_buffer)
            self.key_insert_times[neg_key] = time.monotonic()
        else:
    ...
    async def set(self, key: str, value: Buffer) -> None:
        ...
        neg_key = self._negative_key(key)
        await self._cache.delete(neg_key)
        self.key_insert_times.pop(neg_key, None)
    ...
    async def delete(self, key: str) -> None:
        ...
        # Also delete any negative cache entry
        neg_key = self._negative_key(key)
        await self._cache.delete(neg_key)
        self.key_insert_times.pop(neg_key, None)
    ...
    def cache_stats(self) -> dict[str, Any]:
        ...
        total_requests = self._hits + self._negative_hits + self._misses
        total_hits = self._hits + self._negative_hits
